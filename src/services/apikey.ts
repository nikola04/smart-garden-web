...
export const getAPIKey = async (deviceId: string): Promise<IAPIKey[]> => {
    const data = await apiFetch<{ keys: IAPIKey[] }>(`/api/device/${deviceId}/key`);
    if(data && data.keys) {
...
export const registerAPIKey = async (deviceId: string): Promise<{ raw: string, key: IAPIKey }|null> => {
    const data = await apiFetch<{ raw: string, key: IAPIKey }>(`/api/device/${deviceId}/key`, { method: "POST" });
    if(data && data.key) {
        const raw = data.key;
...
export const deleteAPIKey = async (deviceId: string, keyId: string): Promise<void> => {
    await apiFetch(`/api/device/${deviceId}/key/${keyId}`, { method: "DELETE" });
}
